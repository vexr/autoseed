...
use crate::crypto::subtle::{ConditionallySelectable, ConditionallyNegatable};
...
impl ConditionallyNegatable for ProjectiveNielsPoint {
    fn conditional_negate(&mut self, choice: Choice) {
        // To negate a point in Niels coordinates, we swap y_plus_x and y_minus_x
        // and negate T2d
        let old_y_plus_x = self.Y_plus_X;
        self.Y_plus_X.conditional_assign(&self.Y_minus_X, choice);
        self.Y_minus_X.conditional_assign(&old_y_plus_x, choice);
        self.T2d.conditional_negate(choice);
    }
}
...
impl ConditionallyNegatable for AffineNielsPoint {
    fn conditional_negate(&mut self, choice: Choice) {
        ...
        self.y_plus_x.conditional_assign(&self.y_minus_x, choice);
        self.y_minus_x.conditional_assign(&old_y_plus_x, choice);
        self.xy2d.conditional_negate(choice);
    }
}
